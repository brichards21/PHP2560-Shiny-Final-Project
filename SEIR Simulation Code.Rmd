...
all_data <- as.data.frame(cbind(Time = 1:days, Exposed = e, Infected = i, 
...
ggplot(df[df$people_type == "Infected", ], aes(x = Time, 
                          y = num_people)) + geom_line() +
...
ggplot(df_b) + geom_line(aes(x = Time, y = num_people, color = people_type, label = max_num)) + theme_minimal()+
...
length(grep(c("Exposed", "Infected", "Quarantined", "Diagnosed", "Recovered"), df_full$people_type))
...
p <- ggplot(df_use, aes(frame = frame)) +
  geom_line(aes(x = Time, y = num_people, color = people_type, label = max_num)) +
  theme_minimal() +
  theme(legend.title = element_blank())
